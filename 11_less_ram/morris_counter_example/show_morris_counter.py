...
if __name__ == "__main__":
    ...
    if args.build:
        ...
        cPickle.dump((values_we_count,trials_of_morris_counting, values_in_morris_counter), open(PICKLE_FILENAME, 'w'))
    else:
        print "Loading from", PICKLE_FILENAME, " writing to", PNG_FILENAME
        (values_we_count,trials_of_morris_counting, values_in_morris_counter) = cPickle.load(open(PICKLE_FILENAME, 'r'))
        plt.figure(1)
        plt.clf()

        plt.plot(values_we_count, values_we_count, '-k', linewidth=3, label="Integer counter (>=4 bytes)")
        for counter, values_in_morris_counter in enumerate(trials_of_morris_counting):
            plt.plot(values_we_count, values_in_morris_counter, '--', alpha=0.8, label="Morris counter {} (1 byte)".format(counter))
