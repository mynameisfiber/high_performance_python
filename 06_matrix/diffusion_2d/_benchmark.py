...
def set_grid_shape(grid_width):
    modules = (diffusion_python, diffusion_python_memory, diffusion_numpy, diffusion_numpy_memory, diffusion_scipy, diffusion_numpy_memory2, diffusion_numpy_memory2_numexpr)
    for m in modules:
        if m is not None:
...
if __name__ == "__main__":
    ...
    for grid_width in sizes:
        ...
        data['numpy+memory+laplace+numexpr'].append(run_experiment(diffusion_numpy_memory2_numexpr, nruns, "numpy+memory2+numexpr", baseline))
